...
class Recta:
    def __init__(self, p1: Punto, p2: Punto):
        if p1.coords == p2.coords:
            raise Exception("Los puntos son coincidentes")
        else:
            self.p1 = p1
            self.p2 = p2
...
class Segmento(Recta):
    ...
    def comprobar_punto_en_segmento(self, p):
        if self.comprobar_punto_en_recta(p):
            if self.distancia_entre_extremos > self.p1.distancia(p) and self.distancia_entre_extremos > self.p2.distancia(p):
                return True
            else:
                return False
        else:
...
class Plano:
    def __init__(self, p1, p2=None, p3=None, normal=None):
        if normal:
            self.p1 = p1
            # self.ecuacion = {"A": normal[0], "B": normal[1], "C": normal[2], "D":a*p1.x + b*p1.y + c*p1.z}
        else:
            self.p1 = p1
            self.p2 = p2
            self.p3 = p3
...
class EntidadGeometrica(QWidget):
    def __init__(self, programa, internal_id: int, nombre: str, sympy):
        ...
        # Vértices del cubo
        self.vertices = (Punto(500, 500, 500), Punto(-500, 500, 500), Punto(-500, -500, 500),
                         Punto(500, -500, 500), Punto(500, 500, -500), Punto(-500, 500, -500),
                         Punto(-500, -500, -500), Punto(500, -500, -500))

        # Aristas del cubo
        self.aristas = (Segmento(self.vertices[0], self.vertices[1]), Segmento(self.vertices[1], self.vertices[2]),
                        Segmento(self.vertices[2], self.vertices[3]), Segmento(self.vertices[3], self.vertices[0]),
                        Segmento(self.vertices[0], self.vertices[4]), Segmento(self.vertices[1], self.vertices[5]),
                        Segmento(self.vertices[2], self.vertices[6]), Segmento(self.vertices[3], self.vertices[7]),
                        Segmento(self.vertices[4], self.vertices[5]), Segmento(self.vertices[5], self.vertices[6]),
                        Segmento(self.vertices[6], self.vertices[7]), Segmento(self.vertices[7], self.vertices[4]))

        # Caras del cubo
        self.planos = (Plano(self.vertices[0], self.vertices[1], self.vertices[2]),
                       Plano(self.vertices[0], self.vertices[1], self.vertices[5]),
                       Plano(self.vertices[0], self.vertices[4], self.vertices[7]),
                       Plano(self.vertices[4], self.vertices[5], self.vertices[7]),
                       Plano(self.vertices[2], self.vertices[3], self.vertices[7]),
                       Plano(self.vertices[1], self.vertices[2], self.vertices[5]))
...
class WidgetRecta(EntidadGeometrica):
    def __init__(self, programa, internal_id: int, nombre: str, sympy: Recta, puntos: list = None):
        ...
        self.e = self.extremos(self.sympy)
        # Extremos de la recta separados por cuadrantes
        self.extremos_I = tuple([i for i in self.e if i.y >= 0 and i.z >= 0])
        self.extremos_II = tuple([i for i in self.e if (i.y < 0 and i.z > 0) or (i.y < 0 and i.z == 0)])
        self.extremos_III = tuple([i for i in self.e if i.y < 0 and i.z < 0])
        self.extremos_IV = tuple([i for i in self.e if (i.y > 0 and i.z < 0) or (i.y == 0 and i.z < 0)])
    ...
    def calcular_traza_h(self):
        ...
        if traza_h:
            if not isinstance(traza_h, Recta):
    ...
    def calcular_traza_v(self):
        ...
        if traza_v:
            if not isinstance(traza_v, Recta):
    ...
    def trazas_entre_puntos(self):
        ...
        if interseccion_pv:
            if not isinstance(interseccion_pv, Segmento):
                if interseccion_pv.z > 0:
                    self.traza_v_entre_puntos = "PV+"
                else:
                    self.traza_v_entre_puntos = "PV-"
        if interseccion_ph:
            if not isinstance(interseccion_ph, Segmento):
...
class WidgetPlano(EntidadGeometrica):
    def __init__(self, programa, internal_id: int, nombre: str, sympy: Plano, puntos: list = None):
        ...
        if self.traza_h and self.traza_v:
            punto_en_LT = intersection(self.sympy, Segmento(Punto(500, 0, 0), Punto(-500, 0, 0)))
            if punto_en_LT:
                if not isinstance(punto_en_LT[0], Segmento):
                    if abs(punto_en_LT[0][0]) <= 500:
                        self.punto_en_LT = punto_en_LT[0]

                    if self.traza_h.x[1] < self.traza_h.y[1]:
                        self.traza_h.x, self.traza_h.y = self.traza_h.y, self.traza_h.x
                    if self.traza_v.x[2] < self.traza_v.y[2]:
                        self.traza_v.x, self.traza_v.y = self.traza_v.y, self.traza_v.x
    ...
    def limites(self):
        ...
        for i in range(12):
            ...
            if inter:
                if not isinstance(inter[0], Segmento):
    ...
    def calcular_partes(self):
        ...
        for segmento in self.plano_vertical_bordes:
            interseccion = plano.interseccion_con_segmento(segmento)
            if interseccion:
                if isinstance(interseccion, Segmento):
                    puntos.append(segmento.p1[0].coords)
                    puntos.append(segmento.p2[1].coords)
                else:
                    puntos.append(interseccion.coords)
        for segmento in self.plano_horizontal_bordes:
            ...
            if interseccion:
                if isinstance(interseccion, Segmento):
                    ...
                    puntos.append(interseccion.coords)
        interseccion = plano.interseccion_con_segmento(Segmento(Punto(500, 0, 0), Punto(-500, 0, 0)))
        if interseccion:
            if isinstance(interseccion, Segmento):
        ...
        if self.vector_normal[0] == 0 and self.vector_normal[1] == 0:
            ...
            for i in range(4):
                if not len(trazas) == 2:
                    ...
                    if interseccion and not isinstance(interseccion[0], Segmento):
                        trazas.append(interseccion[0].coords)
